...
// On the mobile shell the Honors panel lives behind a tab — switch to it,
// then scroll the unlocked row into view so it's ready to claim.
function jumpToAchievement(id) {
  if (typeof selectTab === "function") selectTab("honors");
  // Two RAFs: one for the tab swap, one for layout to settle before scrolling.
...
const FX_MAX_LOOT_DOTS  = 6;
const FX_MAX_VALUE_POPS = 4;
...
const SLOT_SYMBOLS = ["🦈", "🌊", "🧜", "🗺", "🌟"];
// Base bonus chances: each positive tier gets +1 over the half-rate floor
// (mini 9→10, minor 6→7, major 2→3, jackpot 1→2); "none" absorbs the rest.
// Shark stays at 8 — it's a hazard.
const SLOT_OUTCOMES = [
  { tier: "none",    weight: 70, pick: () => slotNonMatch() },
  { tier: "shark",   weight: 8,  pick: () => ["🦈", "🦈", "🦈"] },
  { tier: "mini",    weight: 10, pick: () => ["🌊", "🌊", "🌊"] },
  { tier: "minor",   weight: 7,  pick: () => ["🧜", "🧜", "🧜"] },
